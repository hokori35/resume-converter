...
_WIN_FILENAME_FORBIDDEN = re.compile(r'[\\/:*?"<>|]')


def default_output_docx_filename(basic_info, *, timestamp=None):
    """
    默认输出文件名：拟聘单位—姓名.docx，例如「计算机与人工智能学院—张三.docx」。
    timestamp 非空时在扩展名前追加 _YYYYMMDD_HHMMSS（文件被占用时的备选名）。
    """
    bi = basic_info or {}
    college = str(bi.get("target_college") or "").strip()
    name = str(bi.get("name") or "").strip() or "未命名"
    stem = f"{college}—{name}" if college else name
    stem = _WIN_FILENAME_FORBIDDEN.sub("", stem).strip() or "未命名"
    if timestamp:
        stem = f"{stem}_{timestamp}"
    return f"{stem}.docx"


def _fill_applied_position_paragraph(doc, position: str) -> None:
    """回填表头「申请岗位：…」，保留模板加粗样式。"""
    ...
    for p in doc.paragraphs:
        ...
        if p.runs:
            if len(p.runs) >= 2:
                if "：" not in p.runs[0].text and ":" not in p.runs[0].text:
                    p.runs[0].text = f"{label}："
                p.runs[1].text = pos
                for r in p.runs[2:]:
                    r.text = ""
            else:
                p.runs[0].text = f"{label}：{pos}"
        else:
            p.text = f"{label}：{pos}"
        return
...
# 推荐语：首行标题（可在「领域专家」后仍有兼任/理事等）+ 换行正文；或专家词后接冒号
_RE_REC_SUBTITLE_BODY = re.compile(
    r"^(?:\d+\.\s*)?(.+?(?:方面专家|领域专家)(?:[，,][^\n（(]*)*)\s*(?:\n+|[：:]\s*)(.+)$",
    re.S,
)
...
def _recommendation_subtitle_and_body_paragraphs(rec, index_one_based):
    ...
    if m:
        sub = m.group(1).strip()
        if not re.match(r"^\d+\.", sub):
            sub = f"{index_one_based}.{sub}"
...
def fill_template(template_path, data_json, output_path=None, compact=False):
    ...
    _fill_applied_position_paragraph(doc, bi.get("applied_position", ""))
    ...
    # 输出文件名：拟聘单位—姓名.docx
    out = output_path or default_output_docx_filename(bi)
    try:
        doc.save(out)
        return out
    except PermissionError:
        # 文件被 Word 占用时，自动落到带时间戳的新文件名，避免再次冲突。
        alt = default_output_docx_filename(
            bi, timestamp=datetime.now().strftime("%Y%m%d_%H%M%S")
        )
        doc.save(alt)
        return alt
