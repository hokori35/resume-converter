...
try:
    from batch_extract_json import parse_pub_info as _parse_pub_info_impl
except ImportError:
...
def _journal_issn_or_isbn_match_start(s):
    """ISSN 或 978-… ISBN 在串中的起始下标；无则 None。"""
    m = re.search(
        r"(?:\d{4}-\d{3}[\dXx](?:-\d{3})?|978-\d{1,5}-\d{1,7}-\d{1,7}-\d{1,3}[Xx]?)",
        s or "",
        re.I,
    )
...
def fill_template(template_path, data_json, output_path=None, compact=False):
    ...
    other_raw = _normalize_other_raw_cn_sections(str(data.get("other", "")).strip())
    ...
    if not other_raw:
        ...
        for ln in other_raw.splitlines():
            ...
            if _other_line_has_style_markup(text):
                for seg, b, u in parse_marked_authors(text):
                    append_run(
                        p,
                        seg,
        ...
        if not parts:
            ...
            is_sub = _is_other_section_subtitle_line(plain_det)
            _apply_other_paragraph_layout(p, is_subtitle=is_sub)
            if _other_line_has_style_markup(other_raw):
                for seg, b, u in parse_marked_authors(other_raw):
                    append_run(
                        p,
                        seg,
                        font_cn="宋体",
            ...
            for idx, txt in enumerate(parts, start=1):
                ...
                if _other_line_has_style_markup(txt):
                    for seg, b, u in parse_marked_authors(txt):
                        append_run(
                            p,
                            seg,
                            font_cn="宋体",
