...
SCHEMA_TEMPLATE = {
    "basic_info": {
        ...
        "target_college": "", "applied_position": "",
    },
...
def extract_applied_position_from_doc(doc) -> str:
    """从表头段落读取「申请岗位：xxx」（如助理教授、特聘副研究员）。"""
    label = "申请岗位"
    for p in doc.paragraphs:
        t = (p.text or "").strip()
        if label not in t:
            continue
        m = re.match(rf"^{re.escape(label)}\s*[：:]\s*(.+)$", t)
        if m:
...
# 论文总体情况：A/B/C 与 A* 分级标签，「类」可省略（如「B： 1篇」「A类： 3篇」）
_PSUM_GRADE_A_LABEL = r"A(?:\s*类)?\s*[：:]"
_PSUM_GRADE_B_LABEL = r"B(?:\s*类)?\s*[：:]"
_PSUM_GRADE_C_LABEL = r"C(?:\s*类)?\s*[：:]"
_PSUM_GRADE_A_STAR_LABEL = r"A\s*[*＊](?:\s*类)?\s*[：:]"
...
def _parse_psum_abc_grade(scope, label_pat, s, total_key, first_key, paren_first_comm):
    """
    在限定段落内解析单条 A/B/C 分级（篇数 + 第一作者），label_pat 已含可选「类」。
    返回是否匹配到任一样式。
    """
    prefix = rf"(?<!\*)(?<![A-Z]){label_pat}"
    m2 = re.search(
        prefix
        + rf"\s*({_NUM})\s*篇"
        + paren_first_comm.replace("@@A@@", _NUM).replace("@@B@@", _NUM),
        scope,
        re.I,
    )
    m1 = re.search(
        prefix
        + rf"\s*({_NUM})\s*篇[（(]\s*({_NUM})\s*篇(?:为一作|为第一作者)[^）)]*[）)]",
        scope,
        re.I,
    )
    m0 = re.search(
        prefix
        + rf"\s*({_NUM})\s*篇[（(]\s*篇(?:为一作|为第一作者)[^）)]*[）)]",
        scope,
        re.I,
    )
    m_bare = re.search(prefix + rf"\s*({_NUM})\s*篇", scope, re.I)
    if m2:
        s[total_key] = _to_num(m2.group(1))
        s[first_key] = _to_num(m2.group(2))
    elif m1:
        s[total_key] = _to_num(m1.group(1))
        s[first_key] = _to_num(m1.group(2))
    elif m0:
        s[total_key] = _to_num(m0.group(1))
        s[first_key] = ""
    elif m_bare:
        s[total_key] = _to_num(m_bare.group(1))
        s[first_key] = ""
    return bool(m2 or m1 or m0 or m_bare)
...
def parse_papers_summary(text):
    ...
    # 兼容「按计算机学会分级统计」「按中国计算机学会分类统计」等写法（如杨帆）
    sp_ccf = re.search(
        r"按\s*(?:中国)?计算机学会(?:推荐目录)?(?:分类|分级)统计",
        text,
        re.I,
    )
    ...
    for q in [1, 2, 3, 4]:
        # 「区」可省略，如「Q1： 2篇」
        q_label = rf"Q{q}\s*区?\s*[：:]"
        m2 = re.search(
            q_label + rf"\s*({_NUM})\s*篇"
        ...
        m_bare = re.search(q_label + rf"\s*({_NUM})\s*篇", jcr_text, re.I)
        if m2:
    ...
    # A*类：单独字段（勿写入「其他」，否则填充时会显示成「其他」）；「类」可省略
    m_as2 = re.search(
        rf"{_PSUM_GRADE_A_STAR_LABEL}\s*({_NUM})\s*篇"
        + _PAREN_FIRST_COMM.replace("@@A@@", _NUM).replace("@@B@@", _NUM),
        school_only,
        re.I,
    )
    m_as = re.search(
        rf"{_PSUM_GRADE_A_STAR_LABEL}\s*({_NUM})\s*篇[（(]\s*({_NUM})\s*篇(?:为一作|为第一作者)[^）)]*[）)]",
        school_only,
        re.I,
    )
    m_as0 = re.search(
        rf"{_PSUM_GRADE_A_STAR_LABEL}\s*({_NUM})\s*篇[（(]\s*篇(?:为一作|为第一作者)[^）)]*[）)]",
        school_only,
        re.I,
    )
    m_as_bare = re.search(
    ...
    if m_as2:
        ...
        s["school_a_star"] = _to_num(m_as_bare.group(1))
        s["school_a_star_first"] = ""

    # 校级 A/B/C：「类」可省略（如「B： 1篇」）
    for pat, key in [
        (_PSUM_GRADE_A_LABEL, "school_a"),
        (_PSUM_GRADE_B_LABEL, "school_b"),
        (_PSUM_GRADE_C_LABEL, "school_c"),
    ]:
        _parse_psum_abc_grade(
            school_only, pat, s, key, f"{key}_first", _PAREN_FIRST_COMM
        )
    ...
    # 中国计算机学会（CCF）：仅在 ccf_only 段匹配 A/B/C；「类」可省略
    for pat, key in [
        (_PSUM_GRADE_A_LABEL, "ccf_a"),
        (_PSUM_GRADE_B_LABEL, "ccf_b"),
        (_PSUM_GRADE_C_LABEL, "ccf_c"),
    ]:
        _parse_psum_abc_grade(
            ccf_only, pat, s, key, f"{key}_first", _PAREN_FIRST_COMM
        )
    m_co2 = re.search(
        rf"{_OTHER_LABEL}\s*[：:]\s*({_NUM})\s*篇"
        + _PAREN_FIRST_COMM.replace("@@A@@", _NUM).replace("@@B@@", _NUM),
...
def _recommendation_clean_content(s):
    return re.sub(r"\n{2,}", "\n", (s or "").replace("|", "\n")).strip()


# 职称行常以「方面专家 / 领域专家」结尾，正文前为换行或冒号（如林坤/张生推荐信）
_REC_TITLE_EXPERT_TAIL = r".+?(?:方面专家|领域专家)"
_REC_BODY_SEP = r"(?:\n+|[：:]\s*)"


def _parse_recommendation_header_line(head):
    """仅解析推荐人标题行（职称可含「领域专家，兼任…理事」等逗号延续）。"""
    h = (head or "").strip()
    if not h:
        return None
    m = re.match(
...
def parse_recommendation(text):
    ...
    兼容：）与逗号之间有空格；序号点后空格；正文前为半角冒号或换行（无冒号）。
    """
    text = text.strip()
    ...
    # 首行标题 + 换行正文（标题可在「领域专家」后仍有兼任/理事等，如错误082）
    if "\n" in text:
        head, tail = text.split("\n", 1)
        head, tail = head.strip(), tail.strip()
        if tail and re.search(r"(?:方面专家|领域专家|教授|副教授|博导)", head):
            ph = _parse_recommendation_header_line(head)
            if ph and (ph.get("name") or ph.get("title") or ph.get("role")):
                return {
                    **ph,
                    "content": _recommendation_clean_content(tail),
                }

    # 「张生（本人博士生导师），南京大学教授，…领域专家\\n林坤同学…」（可无序号、可无冒号）
    m_ex_role = re.match(
        rf"^(?:\d+\.\s*)?(.+?)\s*[（(]\s*(.+?)\s*[）)]\s*[，,]\s*"
        rf"({_REC_TITLE_EXPERT_TAIL})\s*{_REC_BODY_SEP}(.*)$",
        text,
        re.S,
    )
    if m_ex_role:
        return {
            "name": m_ex_role.group(1).strip(),
            "role": m_ex_role.group(2).strip(),
            "title": m_ex_role.group(3).strip().rstrip("，,"),
            "content": _recommendation_clean_content(m_ex_role.group(4)),
        }
    # 「2. 陈海懿，南京大学副教授，…领域专家 \\n正文」（无括号角色）
    m_ex_plain = re.match(
        rf"^(?:\d+\.\s*)?([^，,（(\n]+?)\s*[，,]\s*"
    ...
    if m_ex_plain:
        return {
            "name": m_ex_plain.group(1).strip(),
            "role": "",
            "title": m_ex_plain.group(2).strip().rstrip("，,"),
            "content": _recommendation_clean_content(m_ex_plain.group(3)),
        }
    # 抽取丢失姓名、仅剩「（本人博士生导师），…专家\\n正文」时
    m_role_only = re.match(
        rf"^[（(]\s*(.+?)\s*[）)]\s*[，,]\s*"
        rf"({_REC_TITLE_EXPERT_TAIL})\s*{_REC_BODY_SEP}(.*)$",
        text,
        re.S,
    ...
    if m_role_only:
        return {
            "name": "",
            "role": m_role_only.group(1).strip(),
            "title": m_role_only.group(2).strip().rstrip("，,"),
            "content": _recommendation_clean_content(m_role_only.group(3)),
        }
    ...
    if m:
        return {
            ...
            "role": m.group(2).strip(),
            "title": m.group(3).strip().rstrip("，,"),
            "content": _recommendation_clean_content(m.group(4)),
        }
    ...
    if m2:
        return {
            ...
            "role": "",
            "title": m2.group(2).strip().rstrip("，,"),
            "content": _recommendation_clean_content(m2.group(3)),
        }
    # 无序号：「姓名（角色），机构，职称：正文」
    m3 = re.match(
        r"(.+?)\s*[（(]\s*(.+?)\s*[）)]\s*[，,]\s*(.+?)[:：]\s*\|?(.*)",
    ...
    if m3:
        return {
            ...
            "content": _recommendation_clean_content(m3.group(4)),
        }
    # 无序号无角色：「姓名，机构职称：正文」
    ...
    if m4:
        return {
            ...
            "content": _recommendation_clean_content(m4.group(3)),
        }
    return {
        "name": "",
        "role": "",
        "title": "",
        "content": _recommendation_clean_content(text),
    }
...
def _pick_recommendation_text_from_row(row, doc):
    """
    推荐人意见格：姓名常在 Word 自动编号 run 中，cell.text 会丢失「1.张生」仅余「（本人博士生导师）…」。
    取同行最长且含逗号的单元格，并用 _cell_text_with_auto_num 补全列表编号。
    """
    _REC_LABELS = {"推荐人意见", "推|荐|人|意|见"}
    best = ""
    for cell in row.cells:
        t = _cell_text_with_auto_num(cell, doc).replace("\r\n", "\n").strip()
        if not t:
            continue
        flat = t.replace("|", "").strip()
        if flat in _REC_LABELS or (flat == "推荐人意见"):
            continue
        if "推荐人意见" in flat and len(flat) <= 12:
            continue
...
def _row_text_for_header_match(txt):
    """
    表头行经 consec_unique 后，竖排单元格会把「起止时间」拆成「起止|时间」，
    导致无法匹配「起止时间」子串（如错误081）。去掉 | 再判表头。
    """
...
def extract_one(docx_path):
    ...
    bi["applied_position"] = extract_applied_position_from_doc(doc)
    ...
    proj_hdr = next(
        (
            i
            for i, txt in enumerate(row_texts)
            if "起止时间" in _row_text_for_header_match(txt)
            and "项目名称" in _row_text_for_header_match(txt)
        ),
        -1,
    )
        ...
        for ridx in range(proj_hdr + 1, proj_end):
            ...
            jd = next(
                (
                    ...
                    if _proj_date_re.search(_strip_vertical_table_noise(c))
                ),
                None,
            )
            if jd is None:
    ...
    for ridx in range(rec_scan_start, row_count):
        ...
        if "推荐人意见" not in txt and "推|荐|人|意|见" not in txt:
            continue
        rec_text = _pick_recommendation_text_from_row(t.rows[ridx], doc)
        if not rec_text:
            ...
            if not rec_text:
                ...
                rec_text = next(
                    (
                        ...
                        for x in cells
                        if x.strip()
                        and x.strip() not in _REC_LABELS
                        and "推荐人意见" not in x
                        and re.search(r"[，,]", x)
                    ),
                    "",
