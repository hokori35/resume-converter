...
def _json_file_response(result_path: Path, out_name: str) -> JSONResponse:
...
async def _handle_convert_resume(
    *,
    ...
    if return_json:
        return _json_file_response(result_path, out_name)
...
def health():
    return {
        ...
        "endpoints": ["/convert-resume", "/convert_resume"],
    }
...
async def convert_resume(
    ...
    JSON 请求默认返回 JSON（含 file_base64）；加 ?response=file 可改回文件流。
    """
